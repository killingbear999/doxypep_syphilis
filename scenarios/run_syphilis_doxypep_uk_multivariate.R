...
get_C_baseline <- function(I, P, S, E) {
  return(I + P + S + E)
}

get_N_baseline <- function(U, I, P, S, E, L, T, R) {
...
doxypep_model <- function(t, y, parameters) {
  with(as.list(c(y, parameters)), {
    ...
    eta_H <- eta_H_random # get_eta(t, t_0, eta_H_init, phi_eta, isFixed)
    eta_L <- omega * eta_H
...
syphilis_model <- function(t, y, parms) {
  with(as.list(c(y, parms)), {
    ...
    # Population sizes
    C_H <- get_C_baseline(I_N_H, P_N_H, S_N_H, E_N_H)
    C_L <- get_C_baseline(I_N_L, P_N_L, S_N_L, E_N_L)
    N_H <- get_N_baseline(U_N_H, I_N_H, P_N_H, S_N_H, E_N_H, L_N_H, T_N_H, R_N_H)
    N_L <- get_N_baseline(U_N_L, I_N_L, P_N_L, S_N_L, E_N_L, L_N_L, T_N_L, R_N_L)
...
run_doxypep <- function(is_p_DbE, is_p_DoD_H, is_p_DoD_L, is_p_DoS_H, is_p_DoS_L) {
  ...
  random_integers <- sample(1:6000, size = n_iter_calibration, replace = FALSE) # draw random integers without replacement
  print(random_integers)
  n_years <- 15
  
  # define number of iterations
  n_iter_multivariate <- 100
  n_iter <- n_iter_calibration * n_iter_multivariate
 
  # define parameters for baseline 
  cases_baseline <- matrix(NA, nrow = n_iter, ncol = n_years)
  cases_baseline_low <- matrix(NA, nrow = n_iter, ncol = n_years)
  cases_baseline_high <- matrix(NA, nrow = n_iter, ncol = n_years)
  cases_baseline_primary <- matrix(NA, nrow = n_iter, ncol = n_years)
  ...
  k = 1
  l = 1
  for (j in 0:n_iter_multivariate-1) {
    # Uptake rate
    uptake = runif(1, min = 0, max = 1)
    p_DbE <- if (is_p_DbE) uptake else 0
    p_DoD_H <- if (is_p_DoD_H) uptake else 0
    p_DoD_L <- if (is_p_DoD_L) uptake else 0
    p_DoS_H <- if (is_p_DoS_H) uptake else 0
    p_DoS_L <- if (is_p_DoS_L) uptake else 0
    
    # Scaling factor accounting for doxycycline inefficacy due to inconsistent and irregular usage
    zeta <- runif(1, min = 0, max = 1)
    
    # Discontinuation rate of doxy-PEP
    xi_N <- runif(1, min = 0.181, max = 0.724)
    
    # Suboptimal adherence rate of doxy-PEP
    xi_X <- runif(1, min = 0.210, max = 0.840)
    
    # Discontinuation rate of doxy-PEP for strata X
    xi_XN <- runif(1, min = 0.468, max = 1.872)
    
    # Screening rate for high-risk group
    eta_H_random <- runif(1, min = 0.200, max = 4.00)
    ...
    for (i in 1:n_iter_calibration) {
      ...
      params <- list(
        ...
        e_d = e_d, zeta = zeta, xi_M = xi_M, xi_N = xi_N, xi_X = xi_X, p_DbE = p_DbE, p_DoD_H = p_DoD_H, p_DoD_L = p_DoD_L,p_DoS_H = p_DoS_H, p_DoS_L = p_DoS_L, xi_XN = xi_XN, eta_H_random = eta_H_random
      )
      ...
      for (t in 1:(n_years)) {
        ...
        incidence_others[t] = 0.5 * ((eta_H_t + params$psi_L) * (out[t, 6] + out[t, 14] + out[t, 22] + out[t, 30]) 
                                     + (eta_H_t + params$psi_T) * (out[t, 7] + out[t, 15] + out[t, 23] + out[t, 31]) 
                                     + (eta_H_t1 + params$psi_L) * (out[t + 1, 6] + out[t + 1, 14] + out[t + 1, 22] + out[t + 1, 30])
                                     + (eta_H_t1 + params$psi_T) * (out[t + 1, 7] + out[t + 1, 15] + out[t + 1, 23] + out[t + 1, 31])
                                     + (eta_L_t + params$psi_L) * (out[t, 38] + out[t, 46] + out[t, 54] + out[t, 62])
                                     + (eta_L_t + params$psi_T) * (out[t, 39] + out[t, 47] + out[t, 55] + out[t, 63]) 
        ...
        C_H <- get_C(out[t, 3], out[t, 4], out[t, 5], out[t, 6], out[t, 11], out[t, 12], out[t, 13], out[t, 14], out[t, 19], out[t, 20], out[t, 21], out[t, 22], out[t, 27], out[t, 28], out[t, 29], out[t, 30])
        C_L <- get_C(out[t, 35], out[t, 36], out[t, 37], out[t, 38], out[t, 43], out[t, 44], out[t, 45], out[t, 46], out[t, 51], out[t, 52], out[t, 53], out[t, 54], out[t, 59], out[t, 60], out[t, 61], out[t, 62])
        N_H <- get_N(out[t, 2], out[t, 3], out[t, 4], out[t, 5], out[t, 6], out[t, 7], out[t, 8], out[t, 9], out[t, 10], out[t, 11], out[t, 12], out[t, 13], out[t, 14], out[t, 15], out[t, 16], out[t, 17],
                     out[t, 18], out[t, 19], out[t, 20], out[t, 21], out[t, 22], out[t, 23], out[t, 24], out[t, 25], out[t, 26], out[t, 27], out[t, 28], out[t, 29], out[t, 30], out[t, 31], out[t, 32], out[t, 33])
        N_L <- get_N(out[t, 34], out[t, 35], out[t, 36], out[t, 37], out[t, 38], out[t, 39], out[t, 40], out[t, 41], out[t, 42], out[t, 43], out[t, 44], out[t, 45], out[t, 46], out[t, 47], out[t, 48], out[t, 49],
                     out[t, 50], out[t, 51], out[t, 52], out[t, 53], out[t, 54], out[t, 55], out[t, 56], out[t, 57], out[t, 58], out[t, 59], out[t, 60], out[t, 61], out[t, 62], out[t, 63], out[t, 64], out[t, 65])
      ...
      cases[k,] <- incidence
      cases_low[k,] <- incidence_low
      cases_high[k,] <- incidence_high
      cases_primary[k,] <- incidence_primary
      cases_secondary[k,] <- incidence_secondary
      cases_others[k,] <- incidence_others
      cases_diagnosed[k,] <- incidence_diagnosed
      prescriptions[k,] <- presctiption
      nondoxy_U[k,] = nondoxy_U_H
      inconsistentdoxy_U[k,] = inconsistentdoxy_U_H
      doxy_U[k,] = doxy_U_H
      intolerantdoxy_U[k,] = intolerantdoxy_U_H
      foi_high[k,] <- lambda_high
      foi_low[k,] <- lambda_low
      k <- k + 1
    }
    ...
    for (i in 1:n_iter_calibration) {
      ...
      for (t in 1:(n_years)) {
        ...
        isFixed = TRUE
        eta_H_t <- 0.184 # get_eta(t+17, t_0, params$eta_H_init, params$phi_eta, isFixed)
        eta_H_t1 <- 0.184 # get_eta(t+17+1, t_0, params$eta_H_init, params$phi_eta, isFixed)
        eta_L_t <- params$omega * eta_H_t
        eta_L_t1 <- params$omega * eta_H_t1
      ...
      cases_baseline_low[l,] <- incidence_low
      cases_baseline_high[l,] <- incidence_high
      cases_baseline_primary[l,] <- incidence_primary
      cases_baseline_secondary[l,] <- incidence_secondary
      cases_baseline_others[l,] <- incidence_others
      cases_baseline_diagnosed[l,] <- incidence_diagnosed
      foi_baseline_high[l,] <- lambda_high
      foi_baseline_low[l,] <- lambda_low
      l <- l + 1
    }
  }
  ...
  # compute total number of averted diagnosed cases
  averted_cases_diagnosed = cases_baseline_diagnosed - cases_diagnosed
  total_averted_cases_diagnosed = rowSums(averted_cases_diagnosed)
  smr_averted_diagnosed = as.data.frame(t(quantile(total_averted_cases_diagnosed, probs = c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm = TRUE)))
  colnames(smr_averted_diagnosed) <- c("X2.5.", "X25.", "X50.", "X75.", "X97.5.")
  ...
  probs <- c(0.025, 0.25, 0.5, 0.75, 0.975)
  smr_pred <- t(apply(cases_baseline, 2, quantile, probs = probs, na.rm = TRUE))
  colnames(smr_pred) <- c("X2.5.", "X25.", "X50.", "X75.", "X97.5.")
  smr_pred <- as.data.frame(smr_pred)
  ...
  return(list(df_combined_cases = df_combined_cases, df_combined_susceptible = df_combined_susceptible, smr_averted_diagnosed = smr_averted_diagnosed,
              ...
              smr_averted_secondary = smr_averted_secondary, smr_averted_others = smr_averted_others, df_foi_high = df_foi_high))
}

############################################################## get results ##############################################################
is_uptake <- TRUE
# DbE
result_DbE <- run_doxypep(is_p_DbE = is_uptake, is_p_DoD_H = FALSE, is_p_DoD_L = FALSE, is_p_DoS_H = FALSE, is_p_DoS_L = FALSE)
...
df_combined_DbE_foi_high <- result_DbE$df_foi_high

# DoD(H)
result_DoD_H <- run_doxypep(is_p_DbE = FALSE, is_p_DoD_H = is_uptake, is_p_DoD_L = FALSE, is_p_DoS_H = FALSE, is_p_DoS_L = FALSE)
...
df_combined_DoD_H_foi_high <- result_DoD_H$df_foi_high

# DoD
result_DoD <- run_doxypep(is_p_DbE = FALSE, is_p_DoD_H = is_uptake, is_p_DoD_L = is_uptake, is_p_DoS_H = FALSE, is_p_DoS_L = FALSE)
...
df_combined_DoD_foi_high <- result_DoD$df_foi_high

# DoA(H)
result_DoA_H <- run_doxypep(is_p_DbE = FALSE, is_p_DoD_H = is_uptake, is_p_DoD_L = FALSE, is_p_DoS_H = is_uptake, is_p_DoS_L = FALSE)
...
df_combined_DoA_H_foi_high <- result_DoA_H$df_foi_high

# DoA
result_DoA <- run_doxypep(is_p_DbE = FALSE, is_p_DoD_H = is_uptake, is_p_DoD_L = is_uptake, is_p_DoS_H = is_uptake, is_p_DoS_L = is_uptake)
...
df_combined_DoA_foi_high <- result_DoA$df_foi_high

# DaR
result_DaR <- run_doxypep(is_p_DbE = FALSE, is_p_DoD_H = is_uptake, is_p_DoD_L = is_uptake, is_p_DoS_H = is_uptake, is_p_DoS_L = FALSE)
...
# save.image("workspace_0.33_fixed_uk.RData")
############################################################## plot FOI ##############################################################
df_combined_DbE_foi_high$scenario <- "DbE"
df_combined_DoD_H_foi_high$scenario <- "DoD(H)"
df_combined_DoD_foi_high$scenario <- "DoD"
df_combined_DoA_H_foi_high$scenario <- "DoA(H)"
df_combined_DoA_foi_high$scenario <- "DoA"
df_combined_DaR_foi_high$scenario <- "DaR"
df_combined_foi <- bind_rows(df_combined_DbE_foi_high, df_combined_DoD_H_foi_high, df_combined_DoD_foi_high, df_combined_DoA_H_foi_high, df_combined_DoA_foi_high, df_combined_DaR_foi_high)
...
ggplot(df_combined_foi, aes(
  ...
  geom_ribbon(
    aes(ymin = lower, ymax = upper, group = scenario),
    alpha = 0.3,
    color = NA
  ) +
  geom_line(
    aes(y = middle, group = scenario),
    linewidth = 0.8
  ) +
  geom_point(
    aes(y = middle),
